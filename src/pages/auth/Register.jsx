...
import { User, ChevronLeft, Bike, FileText, Car, Mail, Lock, Eye, EyeOff, Phone } from 'lucide-react';
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <Field label="Full Name">
        <input value={name} onChange={e => { setName(e.target.value); setErrs(r => ({ ...r, name: '' })); }}
          placeholder="e.g. Mariama Kamara"
          className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 ${errs.name ? 'border-red-400' : 'border-gray-200'}`} />
        {errs.name && <p className="text-red-500 text-xs mt-1">{errs.name}</p>}
      </Field>
      <Field label="Email">
        <div className="relative">
          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
          <input type="email" value={email} onChange={e => { setEmail(e.target.value); setErrs(r => ({ ...r, email: '' })); }}
            placeholder="you@example.com" autoComplete="email"
            className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 ${errs.email ? 'border-red-400' : 'border-gray-200'}`} />
        </div>
      ...
      <Field label="Password">
        <div className="relative">
          ...
          <input type={show ? 'text' : 'password'} value={password}
            onChange={e => { setPassword(e.target.value); setErrs(r => ({ ...r, password: '' })); }}
            placeholder="At least 6 characters" autoComplete="new-password"
            className={`w-full pl-10 pr-10 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 ${errs.password ? 'border-red-400' : 'border-gray-200'}`} />
      ...
      <Field label={<span>Phone <span className="text-gray-400 font-normal">(optional)</span></span>}>
        <div className="relative">
...
// ── Rider form (3 steps) ──────────────────────────────────────────────────────
function RiderForm({ onSubmit, loading, error }) {
  const [step, setStep] = useState(1);
  const [show, setShow] = useState(false);
  const [form, setForm] = useState({
    name: '', email: '', password: '', phone: '',
    motorcycle: '', plate: '', licenseNumber: '',
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-6">
        {['Account', 'Vehicle', 'Documents'].map((label, idx) => (
          <div key={label} className="flex-1">
            <div className={`h-1.5 rounded-full transition-colors ${idx + 1 <= step ? 'bg-orange-500' : 'bg-gray-200'}`} />
            <p className={`text-xs mt-1 text-center ${idx + 1 === step ? 'text-orange-500 font-medium' : 'text-gray-400'}`}>{label}</p>
          </div>
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <Field label={<span>Phone <span className="text-gray-400 font-normal">(optional)</span></span>}>
            <div className="relative">
              <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input type="tel" value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+231 77 000 0000"
                className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500" />
            </div>
          </Field>
          <Field label="Driver License Number">
            <input value={form.licenseNumber} onChange={e => set('licenseNumber', e.target.value)} placeholder="e.g. LB-DL-00123"
              className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 ${errs.licenseNumber ? 'border-red-400' : 'border-gray-200'}`} />
            {errs.licenseNumber && <p className="text-red-500 text-xs mt-1">{errs.licenseNumber}</p>}
          </Field>
          <button onClick={() => validateStep1() && setStep(2)}
            className="w-full bg-orange-500 text-white py-3 rounded-xl font-semibold hover:bg-orange-600">Next →</button>
        </div>
      )}
        ...
        <div className="space-y-4">
          <Field label="Motorcycle Model">
            <input value={form.motorcycle} onChange={e => set('motorcycle', e.target.value)} placeholder="e.g. Honda CB150"
              className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 ${errs.motorcycle ? 'border-red-400' : 'border-gray-200'}`} />
            {errs.motorcycle && <p className="text-red-500 text-xs mt-1">{errs.motorcycle}</p>}
          </Field>
          <Field label="Plate Number">
            <input value={form.plate} onChange={e => set('plate', e.target.value)} placeholder="e.g. LB-2301"
              className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 ${errs.plate ? 'border-red-400' : 'border-gray-200'}`} />
            {errs.plate && <p className="text-red-500 text-xs mt-1">{errs.plate}</p>}
          </Field>
          <div className="flex gap-3">
            ...
            <button onClick={() => validateStep2() && setStep(3)} className="flex-1 bg-orange-500 text-white py-3 rounded-xl font-semibold hover:bg-orange-600">Next →</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => onSubmit({
              ...
              phone: form.phone.trim(), role: 'rider',
              motorcycle: form.motorcycle.trim(), plate: form.plate.trim().toUpperCase(),
              licenseNumber: form.licenseNumber.trim(), nationalId: form.nationalId,
              licenseImg: form.licenseImg, motorcycleImg: form.motorcycleImg, profileImg: form.profileImg,
...
export default function Register() {
  ...
  async function handleSubmit({ password, ...profile }) {
    setLoading(true);
    setError('');
    try {
      await signUp({ ...profile, password });
      if (profile.role === 'rider') navigate('/rider/pending', { replace: true });
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-white flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          ...
          {role === 'passenger' && <PassengerForm onSubmit={handleSubmit} loading={loading} error={error} />}
          {role === 'rider'     && <RiderForm     onSubmit={handleSubmit} loading={loading} error={error} />}
          {role === 'admin'     && <PassengerForm onSubmit={d => handleSubmit({ ...d, role: 'admin' })} loading={loading} error={error} />}
