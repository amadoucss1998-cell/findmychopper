...
const PICKUP = 'Sinkor'; // default current location

function MapView({ activeTrip, nearbyCount }) {
  return (
    <div className="relative h-[52vh] bg-gradient-to-br from-slate-100 to-slate-200 overflow-hidden">
      ...
      <svg className="absolute inset-0 w-full h-full opacity-25">
        ...
        <line x1="0" y1="72%" x2="100%" y2="64%" stroke="#94a3b8" strokeWidth="2.5"/>
      </svg>

      {/* Rider dots */}
      {nearbyCount > 0 && [
        { left: '22%', top: '28%' }, { left: '48%', top: '35%' }, { left: '70%', top: '25%' }
      ].slice(0, Math.min(nearbyCount, 3)).map((pos, i) => (
      ...
      <div className="absolute left-1/2 top-[52%] -translate-x-1/2 -translate-y-1/2">
        ...
        <div className="w-10 h-10 bg-blue-400 opacity-25 rounded-full absolute -top-3 -left-3 animate-ping"></div>
      </div>
      ...
      {activeTrip && activeTrip.status !== 'requested' && (
        <div className="absolute inset-x-4 bottom-4">
          <div className="bg-white/95 backdrop-blur rounded-xl px-4 py-2.5 shadow-lg flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full animate-pulse ${activeTrip.status === 'started' ? 'bg-green-500' : 'bg-orange-500'}`}></div>
            <span className="text-sm font-medium text-gray-700">
              {activeTrip.status === 'accepted' && 'Rider is on the way…'}
              {activeTrip.status === 'arrived'  && 'Rider has arrived!'}
      ...
      <div className="absolute top-4 right-4">
        <div className="bg-white rounded-xl px-3 py-1.5 shadow flex items-center gap-1.5">
          <div className={`w-2 h-2 rounded-full ${nearbyCount > 0 ? 'bg-green-500' : 'bg-gray-300'}`}></div>
          <span className="text-xs font-medium text-gray-700">{nearbyCount} available</span>
        </div>
      </div>
    </div>
  );
...
function FareEstimate({ pickup, destination, onConfirm, onCancel }) {
  const pLoc = locationByName(pickup);
  const dLoc = locationByName(destination);
  const dist = haversine(pLoc.lat, pLoc.lng, dLoc.lat, dLoc.lng);
...
function SearchingCard({ onCancel }) {
  return (
    <div className="text-center py-6">
      ...
      <p className="text-xs text-gray-300">This may take a moment</p>
      <button onClick={onCancel} className="mt-5 text-sm text-red-500 hover:underline">Cancel search</button>
    </div>
  );
}
...
function RiderCard({ trip, onCancel }) {
  const statusConfig = {
    accepted: { label: 'Rider on the way',   badge: 'bg-orange-100 text-orange-600', badgeText: '~3 min away', pulse: 'bg-orange-500' },
    arrived:  { label: 'Rider has arrived!', badge: 'bg-green-100 text-green-700',   badgeText: 'Look for your rider', pulse: 'bg-green-500 animate-pulse' },
    started:  { label: 'Trip in progress',   badge: 'bg-blue-100 text-blue-700',     badgeText: 'On your way', pulse: 'bg-blue-500 animate-pulse' },
  };
  const s = statusConfig[trip.status] || statusConfig.accepted;

  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        <div className={`w-2.5 h-2.5 rounded-full ${s.pulse}`}></div>
      ...
      <div className="flex items-center gap-3 pb-4 mb-4 border-b border-gray-100">
        <div className="w-13 h-13 w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-xl font-bold text-orange-500 flex-shrink-0">
          {trip.riderName?.[0] || '?'}
        </div>
        <div className="flex-1">
          <p className="font-semibold text-gray-900">{trip.riderName}</p>
          <div className="flex items-center gap-1 text-sm text-gray-500 mt-0.5">
            {trip.riderRating > 0 && <><Star className="w-3.5 h-3.5 text-yellow-400 fill-yellow-400" /><span>{trip.riderRating}</span><span className="text-gray-300">·</span></>}
  ...
  return (
    <div>
      ...
      <div className="flex justify-center gap-2 mb-4">
        {[1,2,3,4,5].map(s => (
          <button key={s} onMouseEnter={() => setHovered(s)} onMouseLeave={() => setHovered(0)} onClick={() => setScore(s)}>
            <Star className={`w-9 h-9 transition-colors ${s <= (hovered || score) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 fill-gray-200'}`} />
          </button>
        ))}
      </div>
      ...
      <button onClick={() => onSubmit(score, comment)}
...
export default function PassengerHome() {
  ...
  // Detect trip completion to show rating
  useEffect(() => {
  ...
  const filtered = LOCATIONS.filter(l => l.name.toLowerCase().includes(search.toLowerCase()) && l.name !== PICKUP);

  function selectDest(name) {
    setDest(name); setSearch(''); setShowFare(true);
  }

  function confirm() {
    setShowFare(false);
    requestRide({ pickup: PICKUP, destination });
  }

  function cancel() {
    cancelTrip(); setShowFare(false); setDest('');
  }

  function submitRating(score, comment) {
    submitPassengerRating(score, comment);
    setShowRating(false); setDest('');
  }
  ...
  return (
    <div className="relative min-h-screen">
      <MapView activeTrip={activeTrip} nearbyCount={available.length} />
      ...
      {/* Bottom sheet */}
      <div className="bg-white rounded-t-3xl -mt-6 relative z-10 shadow-2xl min-h-[48vh] px-5 pt-5 pb-6">
        ...
        {isIdle && (
          <>
            ...
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
              ...
              <span>{PICKUP}, Monrovia (your location)</span>
            </div>
            ...
            <div className="space-y-0.5">
              {(search ? filtered : filtered.slice(0, 6)).map(loc => (
        ...
        {showFare && !activeTrip && (
          <FareEstimate pickup={PICKUP} destination={destination} onConfirm={confirm} onCancel={cancel} />
        )}
        ...
        {showRating && activeTrip && (
          <RatingCard trip={activeTrip} onSubmit={submitRating} />
        )}
      </div>
    </div>
  );
